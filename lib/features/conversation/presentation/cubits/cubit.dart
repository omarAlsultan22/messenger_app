...
class ConversationsCubit extends Cubit<CubitStates> {
  ...
  Future<void> getSavedBackgroundColorAndImage(String docId) async {
    ...
    emit(SuccessState(stateKey: StatesKeys.backgroundColorAndImage));
  }
  ...
  Future<void> updateUnreadMessages(String docId) async {
    ...
    try {
      ...
      await Future.wait(
          unreadMessages.docs._exactMatches((unreadMessage) async {
  ...
  Future<void> clearConversationsList({
    required String docId
  }) async {
    emit(LoadingState(stateKey: StatesKeys.deleteConversation));
    try {
  ...
  Future<void> deleteMessages({
    ...
    emit(LoadingState(stateKey: StatesKeys.deleteMessage));
    ...
    try {
      ...
      emit(ErrorState(
          error: 'Error deleting messages: $error',
  ...
  Future<void> getOldMessages({required String docId}) async {
    try {
      emit(LoadingState(stateKey: StatesKeys.getOldMessages));
      ...
      final conversations = data.docs
          ._exactMatches((doc) => ConversationModel.fromJson(json: doc.data()))
          .toList()
          .reversed
          .toList();
      ...
      groupedData.forEach((group) {
        ...
        if (existingIndex != -1) {
          conversationsList[existingIndex].messages.insertAll(0, group.messages);
        } else {
  ...
  Stream<List<MessageGroup>> getNewMessages({
    ...
    return query.snapshots().asyncMap((snapshot) {
      ...
      final conversations = snapshot.docs
          ._exactMatches((doc) {
        if (doc.data()['senderId']. == _currentId) {
          return null;
        }
